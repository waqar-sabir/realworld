...
import { unDislikedArticle, dislikeArticle } from '../services/article.service';
...
export default function DislikeButton({ article, isExtended }: Props) {
  const { user } = useContext(AuthContext);
  const navigate = useNavigate();

  const [count, setCount] = useState(article.dislikedCount);
  ...
  async function updateCount(): Promise<void> {
    ...
    if (disliked) {
      ...
      await unDislikedArticle(article.slug);
    } else {
      setCount(count => count + 1);
  ...
  if (isExtended) {
    return (
      <button
        className={`btn btn-sm btn-${!disliked ? 'outline-' : ''}primary`}
        onClick={updateCount}
        disabled={isLoading}
      >
        ...
        {disliked ? 'Unfavorite' : 'Favorite'} Article <span className="counter">({count})</span>
      </button>
    );
  } else {
    return (
      ...
      >
        ...
        <span style={{ marginRight: '3px' }}>{count}</span>
      </button>
    );
  }
}
